...
CREATE DEFINER=`root`@`localhost` PROCEDURE `proc_get_seat_labels_by_ids` (IN `in_ids` VARCHAR(255))   BEGIN
    SELECT seat_id,
           CONCAT(row_char, real_seat_number) AS seat_label,
           row_char,
           real_seat_number AS seat_number
    FROM seats
    WHERE FIND_IN_SET(seat_id, in_ids);
END$$
